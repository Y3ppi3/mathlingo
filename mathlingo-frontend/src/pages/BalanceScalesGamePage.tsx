// src/pages/BalanceScalesGamePage.tsx
//
// Обвязка игры «Уравнение-весы» (Ф4): выбор уровня, прогресс с бэкенда и
// телеметрия. Сам игровой цикл — в BalanceScalesGame; здесь сеть.
//
// Одна сессия телеметрии на заход: level_start открывает сессию (сервер
// возвращает session_id), move_made/level_complete/level_abandon идут в неё,
// завершение закрывает. Всё через общий api-инстанс — CSRF проставляется
// автоматически, поэтому очки/события не теряются на 403.
//
// game_id = 'balance-scales'; метрика рекорда — число промахов и подсказок
// (меньше лучше), как у «Стрелки Судьбы» метрика — число подсмотренных тиков.
//
...
import { BALANCE_LEVELS } from '../games/balanceScales/levels';
import { opLabel, type Op } from '../games/balanceScales/engine';
import {
    getGameProgress, postGameEvents, upsertGameProgress,
    type GameEvent, type GameProgress,
} from '../api/studentApi';

const GAME_ID = 'balance-scales' as const;

const BalanceScalesGamePage = () => {
    ...
    const startLevel = (index: number) => {
        const level = BALANCE_LEVELS[index];
    ...
    // Пишем и полезные ходы, и промахи: по разбору промахов видно, какие
    // операции школьники путают чаще всего (аналитика игр, admin/games-analytics).
    const handleMove = (op: Op, helpful: boolean) => {
        if (activeIndex === null) return;
        const level = BALANCE_LEVELS[activeIndex];
        emit([{
            event_type: 'move_made',
            payload: { level_id: level.level_id, op: opLabel(op), helpful },
    ...
    const handleHint = () => {
        if (activeIndex === null) return;
        const level = BALANCE_LEVELS[activeIndex];
        emit([{ event_type: 'hint_used', payload: { level_id: level.level_id }, client_ts: new Date().toISOString() }]);
    };

    const handleComplete = async (result: { mistakes: number; hints: number; stars: 1 | 2 | 3; timeMs: number }) => {
        if (activeIndex === null) return;
        const level = BALANCE_LEVELS[activeIndex];
        const metric = result.mistakes + result.hints;
        await emit(
            [{
                event_type: 'level_complete',
                payload: {
                    level_id: level.level_id, mistakes: result.mistakes,
                    hints: result.hints, stars: result.stars, time_ms: result.timeMs,
                },
                client_ts: new Date().toISOString(),
            }],
            true,
        ...
        try {
            const saved = await upsertGameProgress(GAME_ID, level.level_id, result.stars, metric);
    ...
    const activeLevel = activeIndex !== null ? BALANCE_LEVELS[activeIndex] : null;
            ...
            <div className="max-w-3xl mx-auto px-4 py-10 mt-16">
                {activeLevel ? (
                    <BalanceScalesGame
                        ...
                        hasNextLevel={activeIndex! < BALANCE_LEVELS.length - 1}
                        onMove={handleMove}
                        onHint={handleHint}
                    ...
                    <>
                        ...
                        <h1 className="mt-3 text-2xl font-semibold text-gray-900 dark:text-white">Уравнение-весы</h1>
                        <p className="mt-2 text-gray-600 dark:text-gray-400">
                            Уравнение — это весы. Делай с обеими чашами одно и то же, пока слева не останется голый x.
                            Выбирай любой уровень — все открыты.
                        </p>
                                        ...
                                        >
                                            <div>
                                                ...
                                                <div className="text-xs text-gray-400 dark:text-gray-500">
                                                    Сложность {level.difficulty} · ходов по-хорошему {level.par}
                                                </div>
                                            </div>
...
export default BalanceScalesGamePage;
