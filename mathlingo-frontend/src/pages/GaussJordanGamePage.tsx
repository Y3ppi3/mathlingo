// src/pages/GaussJordanGamePage.tsx
//
// Обвязка игры «Побег Гаусса-Жордана» (Фаза 2): выбор уровня, прогресс с
// бэкенда и телеметрия. Сам игровой цикл — в GaussJordanGame; здесь сеть.
//
...
// завершение закрывает сессию (end_session). Всё через общий api-инстанс —
...
const GAME_ID = 'gauss_jordan' as const;

const GaussJordanGamePage = () => {
    ...
    const startLevel = (index: number) => {
        const level = GAUSS_JORDAN_LEVELS[index];
    ...
    const handleMove = (opType: RowOpType) => {
        if (activeIndex === null) return;
        const level = GAUSS_JORDAN_LEVELS[activeIndex];
        emit([{ event_type: 'move_made', payload: { level_id: level.level_id, op: opType }, client_ts: new Date().toISOString() }]);
    };

    const handleComplete = async (result: { moves: number; stars: 1 | 2 | 3; timeMs: number }) => {
        if (activeIndex === null) return;
        const level = GAUSS_JORDAN_LEVELS[activeIndex];
        await emit(
            [{
                event_type: 'level_complete',
                payload: { level_id: level.level_id, moves: result.moves, stars: result.stars, time_ms: result.timeMs },
                client_ts: new Date().toISOString(),
            }],
            true,
        );
        try {
            const saved = await upsertGameProgress(GAME_ID, level.level_id, result.stars, result.moves);
    ...
    const handleNextLevel = () => {
        ...
        if (next < GAUSS_JORDAN_LEVELS.length) {
            startLevel(next);
        } else {
            setActiveIndex(null);
        }
    ...
    const activeLevel = activeIndex !== null ? GAUSS_JORDAN_LEVELS[activeIndex] : null;
            ...
            <div className="max-w-3xl mx-auto px-4 py-10 mt-16">
                {activeLevel ? (
                    <GaussJordanGame
                        ...
                        hasNextLevel={activeIndex! < GAUSS_JORDAN_LEVELS.length - 1}
                        onMove={handleMove}
                    ...
                    <>
                        ...
                        <h1 className="mt-3 text-2xl font-semibold text-gray-900 dark:text-white">Побег Гаусса-Жордана</h1>
                        <p className="mt-2 text-gray-600 dark:text-gray-400">
                            Обращайте матрицу элементарными преобразованиями строк. Выберите уровень — все открыты.
                        </p>

                        <ul className="mt-6 space-y-3">
                            {GAUSS_JORDAN_LEVELS.map((level, index) => {
                                        ...
                                        >
                                            <div>
                                                ...
                                                <div className="text-xs text-gray-400 dark:text-gray-500">
                                                    Сложность {level.difficulty} · пар {level.par}
                                                </div>
                                            </div>
...
export default GaussJordanGamePage;
