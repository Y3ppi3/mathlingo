// src/pages/NumberLineGamePage.tsx
//
// Обвязка игры «Числовая прямая» (Ф4): выбор блока, прогресс с бэкенда и
// телеметрия. Сам игровой цикл — в NumberLineGame; здесь сеть.
//
...
// автоматически.
//
// game_id = 'number-line'; метрика рекорда — средний промах в сотых деления
// (меньше лучше).
//
// Navbar здесь не рендерим: App.tsx ставит его глобально.
import { useCallback, useEffect, useRef, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import NumberLineGame from '../components/games/NumberLineGame';
import { NUMBER_LINE_LEVELS } from '../games/numberLine/levels';
import { type RoundResult } from '../games/numberLine/engine';
import {
    getGameProgress, postGameEvents, upsertGameProgress,
    type GameEvent, type GameProgress,
} from '../api/studentApi';

const GAME_ID = 'number-line' as const;

const NumberLineGamePage = () => {
    ...
    const startLevel = (index: number) => {
        const level = NUMBER_LINE_LEVELS[index];
    ...
    // Пишем каждый раунд с промахом и стороной: по разбору видно, какие числа
    // школьники ставят не туда (например, что −2,5 уезжает правее −2).
    const handleRound = (result: RoundResult) => {
        if (activeIndex === null) return;
        const level = NUMBER_LINE_LEVELS[activeIndex];
        emit([{
            ...
            payload: {
                ...
                guess: result.guess,
                error: Math.round(result.error * 1000) / 1000,
                grade: result.grade,
            },
    ...
    const handleComplete = async (result: { stars: 1 | 2 | 3; metric: number; timeMs: number }) => {
        if (activeIndex === null) return;
        const level = NUMBER_LINE_LEVELS[activeIndex];
        await emit(
            [{
                event_type: 'level_complete',
                payload: { level_id: level.level_id, stars: result.stars, metric: result.metric, time_ms: result.timeMs },
                client_ts: new Date().toISOString(),
            }],
            true,
        );
        try {
            const saved = await upsertGameProgress(GAME_ID, level.level_id, result.stars, result.metric);
    ...
    const handleNextLevel = () => {
        ...
        if (next < NUMBER_LINE_LEVELS.length) {
            startLevel(next);
        } else {
            setActiveIndex(null);
        }
    ...
    const activeLevel = activeIndex !== null ? NUMBER_LINE_LEVELS[activeIndex] : null;
            ...
            <div className="max-w-3xl mx-auto px-4 py-10 mt-16">
                {activeLevel ? (
                    <NumberLineGame
                        ...
                        hasNextLevel={activeIndex! < NUMBER_LINE_LEVELS.length - 1}
                        onRound={handleRound}
                    ...
                    <>
                        ...
                        <h1 className="mt-3 text-2xl font-semibold text-gray-900 dark:text-white">Числовая прямая</h1>
                        <p className="mt-2 text-gray-600 dark:text-gray-400">
                            Ставь число в его точку на прямой. Считать не нужно — нужно почувствовать, где оно живёт.
                            Выбирай любой блок — все открыты.
                        </p>

                        <ul className="mt-6 space-y-3">
                            {NUMBER_LINE_LEVELS.map((level, index) => {
                                        ...
                                        >
                                            <div>
                                                ...
                                                <div className="text-xs text-gray-400 dark:text-gray-500">
                                                    {level.hint} · {level.rounds} раундов
                                                </div>
                                            </div>
...
export default NumberLineGamePage;
